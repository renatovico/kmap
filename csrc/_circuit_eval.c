 ...
 * ================================================================
 *
 * For decode (M=1), this is a memory-bandwidth optimisation:
 * reading W as int8 is 4x less data than float32.
 *
 *   x:      (M, K) float32  — activation
 *   W:      (K, N) int8     — quantized weight (contiguous, row-major)
 *   scales: (N,)   float32  — per-output-column scale
 *   out:    (M, N) float32
 *
 *   out[i,j] = sum_k(x[i,k] * W[k,j]) * scales[j]
 *
 * SIMD strategy (k-outer, j-inner with vector j):
 *   For each k, broadcast x[k] and multiply against 16 (NEON) or
 *   8 (AVX2) int8 values from W[k,:], accumulating in float32
 *   registers.  W is accessed row-by-row (contiguous, cache-friendly).
 */
...
void ceval_matmul_q8(float *out,
    ...
    for (int i = 0; i < M; i++) {
        ...
        /* k-outer, j-inner: W accessed contiguously row-by-row.
         * The inner j-loop is auto-vectorized by the compiler
         * (clang/gcc -O3 -march=native generates NEON/AVX2). */
        for (int k = 0; k < K; k++) {
            float xk = x_row[k];
